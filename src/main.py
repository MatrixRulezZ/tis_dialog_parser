...
def get_user(telegram_id):
    ...
    conn.close()
    if row:
        return {"telegram_id": row[0], "chat_id": row[1], "tis_login": row[2],
                "tis_password": row[3], "last_balance": row[4], "last_traffic_gb": row[5], "last_ip": row[6]}
    return None

def save_user(telegram_id, chat_id, tis_login, tis_password):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute('INSERT OR REPLACE INTO users (telegram_id, chat_id, tis_login, tis_password) VALUES (?, ?, ?, ?)',
...
def update_user_stats(telegram_id, balance, traffic_gb, ip):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    cursor.execute('UPDATE users SET last_balance=?, last_traffic_gb=?, last_ip=? WHERE telegram_id=?',
...
class TISClient:
    ...
    async def login(self):
        try:
            ...
            async with self.session.post("https://stats.tis-dialog.ru/index.php", data=data): pass
...
async def status(message):
    user = get_user(message.from_user.id)
    if not user:
        await bot.send_message(message.chat.id, "Сначала подключи кабинет"); return
...
async def pay(message):
    user = get_user(message.from_user.id)
    if not user: await bot.send_message(message.chat.id, "Сначала подключи кабинет"); return
    client = TISClient(user["tis_login"], user["tis_password"])
    ...
    while True:
        try:
            ...
            for telegram_id, chat_id, login, password, last_ip in users:
                ...
                if data["balance"] < 0:
                    try: await bot.send_message(chat_id, "⚠️ Баланс ушёл в минус!")
                    except: pass
                if data["ip"] != "Н/Д" and last_ip and data["ip"] != last_ip:
                    try: await bot.send_message(chat_id, f"🌐 IP изменился: `{data['ip']}`", parse_mode="Markdown")
                    except: pass
